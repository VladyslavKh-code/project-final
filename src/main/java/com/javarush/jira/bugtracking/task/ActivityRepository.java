...
public interface ActivityRepository extends BaseRepository<Activity> {
    ...
    @Query("SELECT a FROM Activity a WHERE a.taskId = :taskId AND a.statusCode = :statusCode ORDER BY a.updated DESC")
    List<Activity> findAllByTaskIdAndStatusCodeOrderByUpdatedDesc(@Param("taskId") long taskId, @Param("statusCode") String statusCode);
}
